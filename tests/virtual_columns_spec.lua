...
describe("virtual columns", function()
  ...
  a.it("keeps virtual values out of buffer text and renders a virtual header", function()
    require("oil").setup({
    ...
    assert.matches("^/%d+%s+long%-directory/$", lines[2])
    assert.matches("^/%d+%s+a%.txt$", lines[3])
    assert.not_matches("directory%s+long%-directory", lines[2])
    assert.not_matches("file%s+a%.txt", lines[3])
    ...
    vim.api.nvim_buf_set_lines(bufnr, 2, 3, true, { lines[3] })
  ...
  a.it("keeps editable columns physical when virtual columns are interleaved", function()
  ...
  a.it("can show headers while columns remain in buffer text", function()
    require("oil").setup({
      adapters = { ["oil-test://"] = "test" },
      columns = { "type" },
      show_header = true,
    ...
    assert.are.equal(1, #headers)
    assert.are.equal("", vim.api.nvim_buf_get_lines(0, 0, 1, true)[1])
    assert.matches("TYPE", headers[1][4].virt_text[1][1])
  end)
