...
---Get the current buffer's oil URL (e.g. "oil:///path/" or "oil-ssh://host/path/")
---@param bufnr? integer
---@return nil|string
...
---@class (exact) oil.OpenSplitOpts : oil.OpenOpts
...
M.toggle = function(dir, opts, cb)
  if vim.w.is_oil_win or vim.bo.filetype == 'oil' then
    M.close()
    if cb then
      cb()
    end
...
M.setup = function(opts)
  ...
  vim.api.nvim_create_user_command('Oil', callback, {
